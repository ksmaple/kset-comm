...
| L7 — 测试执行层（Test） | 运行 API 可调用性测试和流程测试 | 编译校验通过后串行执行；细则见 [test-execution-spec.md](test-execution-spec.md) |
...
|---|---|
| **输入** | 编译通过的代码 + 生成的测试类 |
| **输出** | 测试执行报告（test-execution-report.json） |
| **依赖** | `TASK-COMPILE-CHECK`（状态必须为 PASSED） |
...
- **测试执行串行**：`TASK-TEST-EXECUTE` 必须在 `TASK-COMPILE-CHECK` 状态为 `PASSED` 后才能执行，作为最后一步。
...
```json
{
  ...
  "testExecution": {
    ...
    "status": "PASSED",
    "apiTests": { "total": 10, "passed": 10, "failed": 0 },
    "flowTests": { "total": 2, "passed": 2, "failed": 0 }
  }
}
```
...
| `TASK-TEST-EXECUTE` | L7 测试执行层 | `TASK-COMPILE-CHECK` | 测试执行报告 |
...
```
PENDING ──→ READY ──→ RUNNING ──→ PASSED ──→ 进入测试
              │           │
...
```
PENDING ──→ READY ──→ RUNNING ──→ PASSED ──→ COMPLETED
              │           │
