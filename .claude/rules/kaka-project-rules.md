...
R005a: 代码注释须使用中文，技术标识符、协议名、类名、方法名、参数名保留原文
...
R037f: `npm run copy:init-skills` 复制文本文件须经 `io.copyFileUtf8NoBom` 写出，确保目标无 BOM
R037g: 可提供 `npm run check:utf8-nobom` 扫描 `.claude/**` 与 `scripts/**`；`-- --fix` 去除已存在的 BOM
...
| 文件首行异常、grep 首列匹配失败 | UTF-8 BOM（EF BB BF） | R037e；`npm run check:utf8-nobom -- --fix` |
...
| 业务仓规则/技能落后于平台 | 未重跑 init 复制 | `npm run copy:init-skills -- <path> --force`（R037f） |
| 三端技能内容不一致 | `.agents`/`.cursor` 实体副本未改链接 | R046；`npm run setup:ai-env-links` |
