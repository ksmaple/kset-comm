...
```
kset-framework/
├── kset-parent/              # 版本 BOM（Boot 3.5.14 / SC 2025.0.2 / SCA 2025.0.0.0 / Dubbo 3.3）
...
├── kset-starter-datasource/  # JDBC + MyBatis-Plus + Flyway 公共数据源能力
...
├── kset-starter-mq/          # RocketMQ 依赖集成 + 事件门面远程实现
...
| `kset-starter-datasource` | `com.kset.datasource` | JDBC、MyBatis-Plus、Flyway、自动填充 |
...
| [docs/maven-nexus.md](docs/maven-nexus.md) | 发布框架库到远端 Nexus |
...
| `kset-starter-datasource` | 逻辑删除约定、createTime/updateTime 自动填充、Flyway 默认路径 | JDBC / MyBatis-Plus / Flyway |
...
Spring 生态版本以 `kset-parent/pom.xml` 为准：Spring Boot 3.5.x 对齐 Spring Cloud 2025.0.x，Spring Cloud Alibaba 使用 2025.0.0.0 版本线。环境中间件版本见 [env/README.md](env/README.md)。
...
### kset-starter-datasource：MyBatis-Plus 与 Flyway
...
`createTime` / `updateTime` 自动填充由 datasource starter 注册；逻辑删除、Mapper 扫描、Flyway 路径使用 MyBatis-Plus / Spring Boot 标准配置扩展。新增配置请使用 `kset.datasource.*`。
