...
├── kset-starter-mq/          # RocketMQ 依赖集成（事件门面远程实现预留）
...
| `kset-starter-mq` | 依赖集成 | RocketMQ starter 依赖；业务事件门面位于 `kset-common` |
...
| [docs/kset-common-utils.md](docs/kset-common-utils.md) | `ListHelper`、`DateHelper`（java.time）、`KsetHttp`、线程池、随机、签名 |
| [docs/kset-cache.md](docs/kset-cache.md) | 多级缓存门面、注解、编程式 API、L1/L2、指标 |
| [docs/kset-redis.md](docs/kset-redis.md) | Redis 统一抽象、强制 TTL、分布式锁、排行榜、多数据源 |
...
| `kset-starter-mq` | RocketMQ 组件依赖入口；事件门面默认在 `kset-common` 提供 Spring 本地实现 | RocketMQ Spring Boot Starter |

## 组件接入总览

| 组件 | Maven 依赖 | 主要入口 | 最小配置/说明 |
|------|------------|----------|---------------|
| 版本基线 | `kset-parent` | Maven parent / BOM | 统一 Java 21、UTF-8、Boot/Cloud/Alibaba/Dubbo 等版本 |
| 公共工具 | `kset-common` | `ListHelper`、`DateHelper`、`KsetHttp`、`KsetThreadPoolFactory`、`StructLog` | 也会由任意 starter 传递引入 |
| 事件门面 | `kset-common` | `EventFacade`、`EventHandler`、`SendCallback` | 默认 Spring 本地事件；MQ 远程实现预留 |
...
| MySQL | `kset-starter-mysql` | MyBatis-Plus Mapper / Entity | 配置 `spring.datasource.*`、`kset.mysql.auto-fill` |
...
| Redis 锁 | `kset-starter-redis` | `KsetRedisLockExecutor`、`@KsetLocked` | 开启 `kset.redis.redisson.enabled=true` |
...
| MQ | `kset-starter-mq` | RocketMQ Spring Boot Starter | 当前提供依赖入口；业务事件优先走 `EventFacade` |
| Cloud SPI | `kset-cloud` | `CloudRuleProvider`、`GrayTagResolver` | starter 内部共享，也可业务扩展 |
| Demo | `kset-demo/*` | standalone / cloud 示例 | 本地启动见 [kset-demo/README.md](kset-demo/README.md) |
...
本节汇总所有框架组件的最小接入方式；完整细节见上方 `docs/*` 文档。
...
### 事件门面：本地 Spring 事件，预留 RocketMQ

事件门面位于 `kset-common`，业务只依赖 `EventFacade`、`EventHandler`、`SendCallback`。默认自动配置为 Spring 本地事件实现，支持普通、异步、延迟、顺序、事务提交后事件。
...
public class OrderService {
    ...
    public OrderService(EventFacade eventFacade) {
...
### kset-starter-mysql：MyBatis-Plus 与 Flyway
...
`createTime` / `updateTime` 自动填充由 starter 注册；逻辑删除、Mapper 扫描、Flyway 路径使用 MyBatis-Plus / Spring Boot 标准配置扩展。
...
Redisson 锁需开启 `kset.redis.redisson.enabled=true`：
...
默认读顺序为 L1 -> L2 -> 方法加载；L2 命中会回填 L1。声明 L2 但没有 L2 适配器时启动失败。
...
kset:
  cloud:
    gateway:
      enabled: true
      route-data-id: demo-gateway-gateway-routes
...
动态路由 JSON 参考 [docs/nacos/demo-gateway-routes.json](docs/nacos/demo-gateway-routes.json)。
...
当前 `kset-starter-mq` 负责提供 `rocketmq-spring-boot-starter` 依赖，版本由 BOM 管理；业务事件推荐先统一使用 `kset-common` 的 `EventFacade`。需要切换为 RocketMQ 远程事件时，在 MQ 模块补充唯一的 RocketMQ 版 `EventFacade` / 消费入口实现，业务发布方和 `EventHandler` 不需要改接口。
